...
int CarModel::ProcessEvent(SimEvent & event)
{
	...
	case EVENT_CARMODEL_RXMESSAGEEND:
	{
		...
		if (bValid)
		{
			...
			case ptSafety:
			{
				printf("safety packets is received");
				SafetyPacket * pMsg = (SafetyPacket *)pPacket;
				//printf("m_iLane = %u m_iHeading = %d", m_iLane, m_iHeading);
				// received message successfully
			//	if(QString((const char*)pMsg->m_pData).contains (QString("192.168.0")))
			//{		printf("no message is transmitted");
			//		break;
//}
				if (!QString((const char*)pMsg->m_pData).contains (QString("@%1 ").arg(IPAddressToString(pMsg->m_ipRX)))){
			        	//printf("m_pData = %s",pMsg->m_pData);
					//if (!QString(IPAddressToString(pMsg->m_ipRX)).contains (QString("192.168.0"))){
						if((const char *)pMsg->m_pData != NULL){
							time_t  timer = time(NULL);
							if(QString(IPAddressToString(pMsg->m_ipRX)).contains (QString("192.168.2"))){
								  char time_string[40]; 
								  struct tm* ptm; 
								  ptm = localtime (&pMsg->m_tTX.tv_sec); 
								  strftime (time_string, sizeof (time_string), "%Y-%m-%d %H:%M:%S", ptm); 
								g_pLogger->LogInfo(QString("[From %1 Receiv %2: %3 from %4 / %5]\n").arg(IPAddressToString(pMsg->m_ID.srcID.ipCar)).arg(IPAddressToString(pMsg->m_ipRX)).arg((const char *)pMsg->m_pData).arg(time_string).arg(ctime(&timer)).ascii());
							}
